...
public class MobileContactApp {
    ...
    static int pivot = -1;
    final static Path path = Paths.get("c:/tmp/Log-Mobile.text");
    ...
    public static void updateController(String oldPhoneNumber, String firstname, String lastname, String newPhoneNumber) {
        try {
            if (oldPhoneNumber == null ||firstname == null || lastname == null || newPhoneNumber == null) {
                throw new IllegalArgumentException("Nulls are nt allowed");
            }
    ...
    public static void updateContactService(String oldPhoneNumber, String fristname, String lastname, String newPhoneNumber) {
        try {
            if (!(update(oldPhoneNumber, fristname, lastname, newPhoneNumber))) {
    ...
    public static boolean isFull(String[][] arr) {
          return pivot == arr.length - 1;  // Μπορούμε και κατευθείαν ως contacts.length - 1 επειδή έχει δηλωθεί αρχικά μέσα στη κλάση
    }
