...
public class CryptoApp {
    ...
    public static int decipher(int cipher, int prev, int key) {
        int de;
        if (prev ==-1) return (char) cipher;

        de = ((cipher - prev + key)) % key; // Προσθέτουμε + key ώστε να μην έχουμε αρνητικό στο mod
        return (char) de;
    }
    ...
    public static ArrayList<Character> decrypt(ArrayList<Integer> encrypted, int key) {
        ...
        while ((token = encrypted.get(i)) != -1) {
            decrypted.add((char) decipher(token, prevToken, key));
            prevToken = token;
            i++;
        }
